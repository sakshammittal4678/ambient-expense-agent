...
def test_pubsub_endpoint(server_fixture: subprocess.Popen[str]) -> None:
    """Test the Pub/Sub push subscription endpoint."""
    import base64
    ...
    encoded_data = base64.b64encode(json.dumps(expense_data).encode("utf-8")).decode(
        "utf-8"
    )
    ...
    pubsub_payload = {
        "message": {
            "data": encoded_data,
            "message_id": "pubsub-msg-12345",
            "publish_time": "2026-06-20T17:00:00Z",
        },
        "subscription": "projects/test-project/subscriptions/expense-topic-sub",
    }

    response = requests.post(
        BASE_URL + "/pubsub", json=pubsub_payload, headers=HEADERS, timeout=15
    )
    assert response.status_code == 200
    res_data = response.json()
    assert res_data["status"] == "success"
    assert res_data["decision"] == "APPROVED"
    assert res_data["subscription"] == "expense-topic-sub"
    assert res_data["session_id"] == "pubsub-msg-12345"
